...
void show_notification(GtkWidget *widget, gpointer data) {
    ...
    if (filtered_announce_count == 0) {
        GtkWidget *no_announcement_label = gtk_label_new("No announcement");
        gtk_widget_set_halign(no_announcement_label, GTK_ALIGN_CENTER);
        gtk_box_pack_start(GTK_BOX(box), no_announcement_label, FALSE, FALSE, 10);
    } else {
        for (int i = 0; i < filtered_announce_count; i++) {
            GtkWidget *message_label = gtk_label_new(filtered_announces[i].content);
            gtk_box_pack_start(GTK_BOX(box), message_label, FALSE, FALSE, 0);
...
GtkWidget* create_header(GtkWidget **buttons, GtkWidget *parent_container) {
    GtkWidget *header, *logo, *menu_box;
    GtkWidget *home_btn, *ticket_btn, *account_btn, *noti_btn;

    // Container chính của Header, set ID để ăn CSS #main-header
    header = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_widget_set_name(header, "main-header"); 
    gtk_widget_set_size_request(header, 1060, 60); 

    // Logo
    logo = gtk_image_new_from_file("../assets/images/logo.png");
    gtk_box_pack_start(GTK_BOX(header), logo, FALSE, FALSE, 20);

    // Menu bên phải
    menu_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 10);
    
    home_btn = gtk_toggle_button_new_with_label("Home");
    ticket_btn = gtk_toggle_button_new_with_label("My Ticket");
    account_btn = gtk_toggle_button_new_with_label("Account");
    noti_btn = gtk_toggle_button_new_with_label("Notification");

    // Đặt ID để nút trong suốt, hover mới hiện màu
    gtk_widget_set_name(home_btn, "header-btn");
    gtk_widget_set_name(ticket_btn, "header-btn");
    gtk_widget_set_name(account_btn, "header-btn");
    gtk_widget_set_name(noti_btn, "header-btn");
    
    gtk_widget_set_size_request(home_btn, 100, 40);
    gtk_widget_set_size_request(ticket_btn, 100, 40);
    gtk_widget_set_size_request(account_btn, 100, 40);
    gtk_widget_set_size_request(noti_btn, 120, 40);

    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(home_btn), TRUE);

    gtk_box_pack_start(GTK_BOX(menu_box), home_btn, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(menu_box), ticket_btn, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(menu_box), account_btn, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(menu_box), noti_btn, TRUE, TRUE, 0);

    // Đẩy menu sang phải
    gtk_box_pack_end(GTK_BOX(header), menu_box, FALSE, FALSE, 20);

    buttons[0] = home_btn;
    buttons[1] = ticket_btn;
    buttons[2] = account_btn;
    buttons[3] = noti_btn;
    buttons[4] = NULL;
    
    g_signal_connect(home_btn, "toggled", G_CALLBACK(on_button_toggled), buttons);
    g_signal_connect(account_btn, "toggled", G_CALLBACK(on_button_toggled), buttons);
    g_signal_connect(ticket_btn, "clicked",G_CALLBACK(show_list_tickets), NULL);
    g_signal_connect(noti_btn, "clicked",G_CALLBACK(show_notification), NULL);
    g_signal_connect(home_btn, "clicked", G_CALLBACK(on_home_button_clicked), NULL);
